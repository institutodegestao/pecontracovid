...
### Suspeitos em Convívio no Pavilhão
...
```{r}
 valueBox(testes_dsp <- base_testes_dsp %>% filter(dt_referencia==max_dt_dsp) %>%  summarise(cont = sum(rt_pcr_dsp)))
```

### Testes Disponíveis - RÁPIDO

```{r}
 valueBox(testes_dsp <- base_testes_dsp %>% filter(dt_referencia==max_dt_dsp) %>%  summarise(cont = sum(rapido_dsp)))
```
